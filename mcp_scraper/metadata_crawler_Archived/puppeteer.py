...
RAW_URL = "https://api.github.com/repos/modelcontextprotocol/servers-archived/contents/src/puppeteer/index.ts"

OUTPUT_DIR = "result_json"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "puppeteer_tools.json")
...
tools_block_match = re.search(
    r'const\s+TOOLS\s*:\s*Tool\[\]\s*=\s*\[\s*([\s\S]*?)\s*\];',
    content
)
...
tool_blocks = re.findall(r'\{\s*name\s*:\s*"[^"]+"\s*,[\s\S]*?\}', tools_block)
